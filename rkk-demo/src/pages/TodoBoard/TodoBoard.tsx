...
function getDueDateInfo(value?: string): {
  ...
  if (!dueDate)
    return { state: "no_due", label: "No due date", helper: "No due date set" };
  ...
  if (days === 0)
    return { state: "today", label: "Today", helper: "Due today" };
  if (days === 1)
    return { state: "tomorrow", label: "Tomorrow", helper: "Due tomorrow" };
  ...
  if (!todo?.priority)
    return (
      <div className="todo-card">
        <div className="todo-card-title">{data.title}</div>
      </div>
    );
  return (
    <div
      className={`todo-card ${
        dueDateInfo.state === "overdue" ? "todo-card--overdue" : ""
      }`}
      onClick={() => onOpen(todo)}
    >
      <div className="todo-card-header">
        <div className="todo-card-priority">
          ...
          />
          <span style={{ color: PRIORITY_COLORS[todo.priority] }}>
            {PRIORITY_LABELS[todo.priority]}
          </span>
          {todo.generatedByAi && (
            <span title="AI Generated" style={{ display: "inline-flex" }}>
              <Sparkles size={14} style={{ color: "#8b5cf6", marginLeft: 4 }} />
            </span>
          )}
        </div>
        <div className="todo-card-actions">
          ...
          <button
            ...
            title={todo.archivedAt ? "Restore" : "Archive"}
          >
            {todo.archivedAt ? <RotateCcw size={14} /> : <Archive size={14} />}
          </button>
          <button
            onClick={(e) => {
              e.stopPropagation();
      ...
      {todo.description && (
        <div className="todo-card-description">{todo.description}</div>
      )}
      {todo.aiSummary && (
        <div className="todo-card-ai-summary">✨ {todo.aiSummary}</div>
      )}
      ...
      <div className="todo-card-footer" style={{ marginTop: 8 }}>
        {todo.dueDate && (
          <div
            className={`todo-card-due todo-card-due--${dueDateInfo.state}`}
            title={parseDueDate(todo.dueDate)?.toLocaleDateString()}
          >
            <Calendar size={12} />
        ...
        {todo.jiraIssueKey && (
          <span
            className={`todo-card-jira-wrap todo-card-jira-${todo.jiraSyncStatus?.toLowerCase()}`}
          >
            <a
              href={todo.jiraIssueUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="todo-card-jira"
              onClick={(e) => e.stopPropagation()}
            >
              🎫 {todo.jiraIssueKey}
            </a>
    ...
    <div className="todo-add-card">
      ...
      <div className="todo-due-picker todo-due-picker--compact">
        ...
        <div className="todo-due-quick-actions">
          <button
            type="button"
            onClick={() => setDueDate(getRelativeDateInputValue(0))}
          >
            Today
          </button>
          <button
            type="button"
            onClick={() => setDueDate(getRelativeDateInputValue(1))}
          >
            Tomorrow
          </button>
          <button
            type="button"
            onClick={() => setDueDate(getRelativeDateInputValue(7))}
          >
            Next week
          </button>
          {dueDate && (
        ...
        {dueDate && (
          <small
            className={`todo-due-helper todo-due-helper--${dueDateInfo.state}`}
          >
            {dueDateInfo.helper}
          </small>
        )}
...
export const TodoBoard: React.FC = () => {
  ...
  const initialBoardCache = selectedProjectId
    ? getBoardCache(selectedProjectId)
    : undefined;
  const [statuses, setStatuses] = useState<TodoStatus[]>(
    () => initialBoardCache?.statuses || [],
  );
  const [todosByStatus, setTodosByStatus] = useState<Map<string, Todo[]>>(
    () => initialBoardCache?.todosByStatus || new Map(),
  );
  const [dataSource, setDataSource] = useState<BoardData | null>(() =>
    initialBoardCache
      ? statusesToBoardData(
          initialBoardCache.statuses,
          initialBoardCache.todosByStatus,
        )
      : null,
  );
  const [isLoading, setIsLoading] = useState(() => !initialBoardCache);
  const [addingCardForColumn, setAddingCardForColumn] = useState<string | null>(
    null,
  );
  ...
  const [jiraAuthType, setJiraAuthType] = useState<JiraAuthType>(
    JiraAuthType.BASIC,
  );
  const [jiraEmail, setJiraEmail] = useState("");
  const [jiraApiToken, setJiraApiToken] = useState("");
  const [jiraProjectKey, setJiraProjectKey] = useState("");
  const [jiraMappings, setJiraMappings] = useState<
  ...
  const [invitePermission, setInvitePermission] =
    useState<ProjectMemberPermission>("READ");
  ...
  const loadData = useCallback(
    async (projectId: string, options: { showLoading?: boolean } = {}) => {
      const { showLoading = true } = options;
      if (showLoading) setIsLoading(true);
      try {
        const statusRes = await todoStatusApi.getAll({
          projectId,
          page: 1,
          limit: 50,
        });
        ...
        if (!hasActiveFilters) {
          setBoardCache(projectId, {
            statuses: fetchedStatuses,
            todosByStatus: todosMap,
          });
        }
      } catch (err) {
        console.error("Failed to load board data:", err);
        showToast(
          "Failed to load board. Please check API connection.",
          "error",
        );
  ...
  useEffect(() => {
    if (selectedProjectId) {
      ...
      if (cachedBoard) {
        ...
        setDataSource(
          statusesToBoardData(cachedBoard.statuses, cachedBoard.todosByStatus),
        );
        setIsLoading(false);
        loadData(selectedProjectId, { showLoading: false });
      } else {
  ...
  const handleOpenJiraSettings = async () => {
    ...
    try {
      ...
      const mappings = await jiraIntegrationApi.getTransitionMappings(
        selectedProjectId,
      );
      setJiraMappings(
        Object.fromEntries(
  ...
  const handleSaveJiraSettings = async () => {
    ...
    try {
      await jiraIntegrationApi.upsert(selectedProjectId, {
        ...
        jiraEmail:
          jiraAuthType === JiraAuthType.BASIC ? jiraEmail.trim() : undefined,
      ...
      const mappings = statuses
        .map((status) => ({
          ...
          jiraTransitionId:
            jiraMappings[status.id]?.jiraTransitionId?.trim() || "",
          jiraTransitionName:
            jiraMappings[status.id]?.jiraTransitionName?.trim() || undefined,
        }))
        .filter((mapping) => mapping.jiraTransitionId);

      await jiraIntegrationApi.upsertTransitionMappings(selectedProjectId, {
        mappings,
      });
      setJiraApiToken("");
      setJiraHasConfig(true);
  ...
  const handleTestJiraConnection = async () => {
    ...
    try {
      ...
      showToast(
        `Connected to Jira${
          result.displayName ? ` as ${result.displayName}` : ""
        }`,
        "success",
      );
    } catch {
  ...
  const handleAddCard = async (data: Omit<CreateTodoReq, "projectId">) => {
    ...
    if (!hasActiveFilters) {
      setBoardCache(selectedProjectId, {
        statuses,
    ...
    try {
      ...
      if (!hasActiveFilters) {
        setBoardCache(selectedProjectId, {
          statuses,
          todosByStatus: previousTodosByStatus,
        });
      }
  ...
  const uploadAttachment = async (todoId: string, file: File) => {
    ...
    await todoAttachmentApi.uploadToStorage(
      presigned.uploadUrl,
      file,
      presigned.headers,
    );
  ...
  const handleUploadTaskAttachments = async (files: FileList | null) => {
    ...
    try {
      const saved = await Promise.all(
        Array.from(files).map((file) => uploadAttachment(editingTodo.id, file)),
      );
      setEditingTodo((current) =>
        current
          ? {
              ...current,
              attachments: [...(current.attachments || []), ...saved],
            }
          : current,
      );
  ...
  const handleAddComment = async () => {
    if (!editingTodo || (!newComment.trim() && newCommentImages.length === 0))
      return;
    if (!canWrite) return;
    ...
    if (imageFiles.length > 0) {
      ...
      try {
        const uploaded = await Promise.all(
          imageFiles.map((file) => uploadAttachment(editingTodo.id, file)),
        );
  ...
  const handleSaveComment = async (commentId: string) => {
    ...
    try {
      const updated = await todoCommentApi.update(editingTodo.id, commentId, {
  ...
  const handleArchiveToggle = async (todo: Todo) => {
    ...
    try {
      ...
      showToast(
        todo.archivedAt ? "Failed to restore task" : "Failed to archive task",
        "error",
      );
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      if (nextJiraIssueKey !== currentJiraIssueKey) {
        await todoApi.linkJiraIssue(editingTodo.id, {
          jiraIssueKey: nextJiraIssueKey,
        });
      }
      setEditingTodo(null);
      if (selectedProjectId) {
  ...
  const handleCardMove = async (move: {
    ...
    if (!canWrite) return;
    const newData = dropHandler(
      move,
      dataSource,
      () => {},
      (newCol) => ({
        ...newCol,
        totalItemsCount: (newCol.totalItemsCount || 0) + 1,
        totalChildrenCount: (newCol.totalChildrenCount || 0) + 1,
      }),
      (srcCol) => ({
        ...srcCol,
        totalItemsCount: (srcCol.totalItemsCount || 0) - 1,
        totalChildrenCount: (srcCol.totalChildrenCount || 0) - 1,
      }),
    );
    ...
    try {
      if (toColumnId !== fromColumnId) {
        const updatedTodo = await todoApi.update(cardId, {
          statusId: toColumnId,
        });
        if (
          updatedTodo.jiraIssueKey &&
          updatedTodo.jiraSyncStatus === "FAILED"
        ) {
          showToast(
            "Card moved, but Jira sync failed. Check the transition id and Jira workflow.",
            "error",
          );
        } else if (
          updatedTodo.jiraIssueKey &&
          updatedTodo.jiraSyncStatus === "PENDING"
        ) {
          showToast(
            "Card moved, but Jira sync is pending. Check Jira integration and status mapping.",
            "warning",
          );
        }
      }
      ...
      showToast("Failed to persist card order", "error");
      loadData(selectedProjectId!);
    }
  };
  ...
  const handleAddColumn = async () => {
    ...
    if (!canWrite) return;
    try {
      const createdStatus = await todoStatusApi.create({
        projectId: selectedProjectId,
  ...
  const handleDeleteColumn = async (columnId: string) => {
    ...
    if (!canWrite) return;
    try {
      await todoStatusApi.delete(columnId);
      showToast("Column deleted", "success");
      loadData(selectedProjectId);
    } catch {
      showToast("Cannot delete column (has cards?)", "error");
    }
  };
  ...
  const handleColumnMove = async (move: {
    ...
    try {
      await Promise.all(
        newOrder.map((colId, idx) =>
          todoStatusApi.update(colId, { order: idx }),
        ),
      );
    } catch {
  ...
  const totalTasks = Array.from(todosByStatus.values()).reduce(
    (a, b) => a + b.length,
    0,
  );
  ...
  const renderAttachments = (
    ...
    return (
      <div
        className={
          compact
            ? "todo-attachment-grid todo-attachment-grid--compact"
          ...
          <div className="todo-attachment" key={attachment.id}>
            ...
            {!compact && (
              <div className="todo-attachment-meta">
                <span title={attachment.originalName}>
                  {attachment.originalName}
                </span>
                {canWrite && (
                  <button
    ...
    <div className="todo-board">
      <div className="todo-board-header">
        ...
        <div className="todo-board-header-right">
          ...
          <button
            className="btn-ghost"
            onClick={handleOpenJiraSettings}
            disabled={!selectedProjectId || !canWrite}
          >
            <Settings size={16} /> Jira Settings
          </button>
          <button
      ...
      <div className="todo-board-filters">
        ...
        <div className="todo-board-filter-group">
          ...
          <select
            ...
            onChange={(e) =>
              setFilterPriority(e.target.value as TodoPriority | "ALL")
            }
          >
          ...
          <select
            value={filterJiraStatus}
            onChange={(e) =>
              setFilterJiraStatus(e.target.value as JiraSyncStatus | "ALL")
            }
          >
        ...
        <div className="todo-add-column-bar">
          ...
          <div className="todo-add-column-colors">
            {DEFAULT_COLUMN_COLORS.map((c) => (
              <button
                key={c}
                className={`todo-color-btn ${
                  newColumnColor === c ? "active" : ""
                }`}
        ...
        {hasColumns ? (
          <Kanban
            ...
            configMap={{
              card: {
                render: ({ data }) => (
                  <TodoCard
                    data={data}
                    onOpen={handleEditTodo}
                    onArchiveToggle={handleArchiveToggle}
                    onDelete={(todo) => canWrite && setDeletingTodo(todo)}
                  />
                ),
            ...
            renderColumnHeader={(column) => {
              const color =
                (column.content as { color?: string })?.color || "#6366f1";
              return (
            ...
            renderListFooter={(column) =>
              addingCardForColumn === column.id ? (
                ...
                <button
                  className="todo-add-card-btn"
                  onClick={() => setAddingCardForColumn(column.id)}
                >
          ...
          >
            ...
            <div className="todo-detail-body">
                    ...
                    <div className="todo-due-picker">
                      ...
                      <div className="todo-due-quick-actions">
                        <button
                          type="button"
                          onClick={() =>
                            setEditDueDate(getRelativeDateInputValue(0))
                          }
                        >
                          Today
                        </button>
                        <button
                          type="button"
                          onClick={() =>
                            setEditDueDate(getRelativeDateInputValue(1))
                          }
                        >
                          Tomorrow
                        </button>
                        <button
                          type="button"
                          onClick={() =>
                            setEditDueDate(getRelativeDateInputValue(7))
                          }
                        >
                          Next week
                        </button>
                      ...
                      <small
                        className={`todo-due-helper todo-due-helper--${editDueDateInfo.state}`}
                      >
                        {editDueDateInfo.helper}
                      </small>
                    </div>
                  </div>
                  <div className="auth-field">
              ...
              <section className="todo-detail-section">
                ...
                <div className="auth-field todo-checkbox-field">
                  <input
                    type="checkbox"
                    checked={editGeneratedByAi}
                    onChange={(e) => setEditGeneratedByAi(e.target.checked)}
                    id="gen-ai"
                    disabled={!canWrite}
                  />
                  ...
                  {editExternalLinks.map((link, idx) => (
                    <div className="todo-detail-link-row" key={idx}>
                      <input
                        ...
                        value={link.name}
                        onChange={(e) => {
                          const newLinks = [...editExternalLinks];
                          newLinks[idx].name = e.target.value;
                          setEditExternalLinks(newLinks);
                        }}
                      />
                      <input
                        type="text"
                      ...
                      <button
                        ...
                        className="btn-ghost"
                        onClick={() => {
                          const newLinks = editExternalLinks.filter(
                            (_, i) => i !== idx,
                          );
                          setEditExternalLinks(newLinks);
                        }}
              ...
              <section className="todo-detail-section">
                ...
                <div className="auth-field">
                  ...
                  <input
                    type="text"
                    value={editJiraIssueKey}
                    onChange={(e) =>
                      setEditJiraIssueKey(e.target.value.toUpperCase())
                    }
                    placeholder="PROJ-123"
                    disabled={!canWrite}
                  />
                  <small>
                    Used to sync status changes with Jira. Leave blank to
                    unlink.
                  </small>
                </div>
                <div className="todo-detail-meta-grid">
                  <div
                    className={`todo-detail-meta-card todo-card-jira-${editingTodo.jiraSyncStatus?.toLowerCase()}`}
                  >
                    <span>Sync status</span>
                    <strong>{editingTodo.jiraSyncStatus}</strong>
                  </div>
                  {editingTodo.jiraIssueUrl && (
                    <a
                      className="todo-detail-jira-link"
                      href={editingTodo.jiraIssueUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      <Link2 size={14} /> Open Jira issue
                    </a>
                  )}
                </div>
              </section>
              ...
              <section className="todo-detail-section">
                ...
                {renderAttachments(
                  (editingTodo.attachments || []).filter(
                    (attachment) => !attachment.commentId,
                  ),
                ) || (
                  <div className="todo-comments-empty">No attachments yet.</div>
                )}
                {canWrite && (
                ...
                <div className="todo-comments">
                  <div className="todo-comment-composer">
                    <textarea
                      ...
                      rows={3}
                      placeholder={
                        canWrite ? "Add a comment..." : "Read-only access"
                      }
                      disabled={!canWrite}
                    />
                    ...
                    <div className="todo-comment-composer-actions">
                      ...
                      <button
                        ...
                        disabled={
                          !canWrite ||
                          attachmentsUploading ||
                          (!newComment.trim() && newCommentImages.length === 0)
                        }
                      >
              ...
              <section className="todo-detail-section">
                ...
                >
                  ...
                  <div
                    className="todo-activity-filters"
                  ...
                  >
                    ...
                    <button
                      className={`btn-ghost ${
                        activityFilter === "TASK_UPDATED" ? "active" : ""
                      }`}
                      style={
                        activityFilter === "TASK_UPDATED"
                          ? { background: "rgba(0,0,0,0.1)" }
                          : {}
                      }
                      onClick={() => setActivityFilter("TASK_UPDATED")}
                    >
                      Updates
                    </button>
                    <button
                      className={`btn-ghost ${
                        activityFilter === "COMMENT_ADDED" ? "active" : ""
                      }`}
                      style={
                        activityFilter === "COMMENT_ADDED"
                          ? { background: "rgba(0,0,0,0.1)" }
                          : {}
                      }
                      onClick={() => setActivityFilter("COMMENT_ADDED")}
                    >
                      Comments
                    </button>
                  </div>
              ...
              <section className="todo-detail-section">
                ...
                <div className="todo-detail-meta-grid">
                  <div className="todo-detail-meta-card">
                    <span>Created</span>
                    <strong>
                      {new Date(editingTodo.createdAt).toLocaleString()}
                    </strong>
                  </div>
                  <div className="todo-detail-meta-card">
                    <span>Updated</span>
                    <strong>
                      {new Date(editingTodo.updatedAt).toLocaleString()}
                    </strong>
                  </div>
                  {editingTodo.archivedAt && (
                    <div className="todo-detail-meta-card">
                      <span>Archived</span>
                      <strong>
                        {new Date(editingTodo.archivedAt).toLocaleString()}
                      </strong>
                    </div>
                  )}
                </div>
            ...
            <div className="todo-detail-footer">
              {canWrite && (
                <button
                  className="btn-ghost"
                  onClick={() => handleArchiveToggle(editingTodo)}
                  disabled={isSavingEdit}
                >
                  {editingTodo.archivedAt ? (
                    <RotateCcw size={16} />
                  ...
                  {editingTodo.archivedAt ? "Restore" : "Archive"}
                </button>
              )}
              {canWrite && (
                <button
                  className="btn-danger"
                  onClick={() => setDeletingTodo(editingTodo)}
                  disabled={isSavingEdit}
                >
              ...
              <div className="todo-detail-footer-actions">
                ...
                <button
                  ...
                  disabled={isSavingEdit || !canWrite}
                >
                  {isSavingEdit ? (
                    <>
        ...
        footer={
          <div className="todo-modal-footer">
            ...
            <button
              ...
              disabled={jiraLoading || !jiraHasConfig}
            >
              Test Connection
            </button>
            <button
      ...
      >
        <div className="todo-jira-settings">
          <div className="todo-jira-section">
            ...
            <p>
              Configure Jira for the selected project only. Leave API token
              blank to keep the existing token.
            </p>
            <div className="todo-edit-row">
              <div className="auth-field">
            ...
            <div className="todo-edit-row">
              <div className="auth-field">
                ...
                <select
                  value={jiraAuthType}
                  onChange={(e) =>
                    setJiraAuthType(e.target.value as JiraAuthType)
                  }
                >
                  <option value={JiraAuthType.BASIC}>Basic</option>
                  <option value={JiraAuthType.BEARER}>Bearer</option>
                </select>
              </div>
          ...
          <div className="todo-jira-section">
            ...
            <p>
              Map each local board column to a Jira transition id. Empty rows
              are ignored.
            </p>
            <div className="todo-jira-mappings">
              {statuses.map((status) => (
                <div className="todo-jira-mapping-row" key={status.id}>
                  <div className="todo-jira-status">
                    <span
                      className="todo-column-dot"
                  ...
                  <input
                    type="text"
                    value={jiraMappings[status.id]?.jiraTransitionId || ""}
                    onChange={(e) =>
                      setJiraMappings((prev) => ({
                        ...prev,
                        [status.id]: {
                          jiraTransitionId: e.target.value,
                          jiraTransitionName:
                            prev[status.id]?.jiraTransitionName || "",
                        },
                      }))
                    }
                  ...
                  <input
                    type="text"
                    value={jiraMappings[status.id]?.jiraTransitionName || ""}
                    onChange={(e) =>
                      setJiraMappings((prev) => ({
                        ...prev,
                        [status.id]: {
                          jiraTransitionId:
                            prev[status.id]?.jiraTransitionId || "",
                          jiraTransitionName: e.target.value,
                        },
                      }))
                    }
              ...
              {statuses.length === 0 && (
                <p>
                  No columns yet. Create columns before adding transition
                  mappings.
                </p>
              )}
            </div>
      ...
      >
        <div className="todo-jira-settings">
          ...
          <div className="todo-jira-section">
            ...
            {membersLoading ? (
              <div className="todo-comments-empty">
                <Spinner size="sm" /> Loading invitations...
              </div>
      ...
      <Modal
        ...
        footer={
          <div className="todo-modal-footer">
            <button className="btn-ghost" onClick={() => setDeletingTodo(null)}>
              Cancel
            </button>
            <button className="btn-danger" onClick={handleDeleteTodo}>
              Delete
            </button>
          </div>
        }
      >
        <p>
          ...
          <strong>"{deletingTodo?.title}"</strong>? This action cannot be
          undone.
        </p>
      </Modal>
    </div>
